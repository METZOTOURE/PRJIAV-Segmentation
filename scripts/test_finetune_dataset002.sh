...
if [ -f "$nnUNet_preprocessed/Dataset002_MSLesSeg_FLAIR/nnUNetResEncUNetLPlans.json" ]; then
    ...
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        nnUNetv2_extract_fingerprint -d 002 --verify_dataset_integrity
        nnUNetv2_move_plans_between_datasets -s 004 -t 002 -sp nnUNetResEncUNetLPlans -tp nnUNetResEncUNetLPlans
        nnUNetv2_plan_experiment -d 002
        nnUNetv2_preprocess -d 002 -plans_name nnUNetResEncUNetLPlans -c 3d_fullres
    else
